...
import AnalyticsMovementReportTable from '../components/AnalyticsMovementReportTable';
import CustomModal from '../components/CustomModal';
import BasicTextFields from '../components/Input';
import '../css/Dashboard.css';
import api from '../services/api';
export class AnalyticsMovementReport extends Component {
    state = {
        location: '',
        device: { value: null },
        openModal: false,
    ...
    async componentDidMount() {
        ...
        if (this.props.location.state) {
            this.setState({ asnName: this.props.location.state })
        } else {
            this.setState({ asnName: '' })
        }
        const Batches = await api.getAssets()
        await this.setState({ Batches: Batches, allData: Batches, searchData: Batches })
        console.log(this.state.Batches, 'adsfasdfasdfasdfsadfadsfasdfds');
        if (Batches) {
                    ...
                    <div className="main-dashboard">
                        ...
                        <div className="dashboard ">
                            <div className="dashboard-header">
                                ...
                                <h1 className="dashboard-heading">Movement Report</h1>
                            </div>
                            ...
                            <AnalyticsMovementReportTable epcEPC={this.state.asnName} data={this.state.Batches} openModal={() => this.handleClickOpen()} />
                        </div>
                    </div>
                </div>
            </React.Fragment>
...
export default AnalyticsMovementReport